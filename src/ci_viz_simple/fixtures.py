...
from ci_viz_simple.main import TestResult
...
def create_fixture_data():
    ...
    flaky_pass_threshold = 3  # Pass 60% of the time (3 out of 5)
    correlation_fail_threshold = 2  # Fail after session 2
    ...
    for i, test_time in enumerate(flaky_times):
        ...
        status = "passed" if i % 5 < flaky_pass_threshold else "failed"
        test_results.append(
            TestResult(
    ...
    for session_num in range(5):  # 5 sessions showing correlation
        ...
        for i, (test_fqn, expected_status) in enumerate(correlation_tests):
            ...
            actual_status = "failed" if i == 1 and session_num > correlation_fail_threshold else expected_status
            ...
            test_results.append(
                TestResult(
                    ...
                    test_traceback=(
                        "AssertionError: Global state not properly reset" if actual_status == "failed" else None
                    ),
...
def load_fixture_data():
    ...
    from ci_viz_simple.main import DB_PATH
...
def load_fixtures_cli():
    ...
    from ci_viz_simple.main import init_database
